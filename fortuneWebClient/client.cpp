    ...
    tcpSocket(new QTcpSocket(this))
{
  ...
  statusLabel = new QLabel(tr("You must run the Fortune Server as well"));
  ...
  connect(hostCombo, &QComboBox::editTextChanged, this, &Client::enableGetFortuneButton);
  ...
  if (manager.capabilities() &QNetworkConfigurationManager::NetworkSessionRequired)
  {
      ...
      getFortuneButton->setEnabled(false);
      statusLabel->setText(tr("Opening network sesion."));
      networkSession->open();
  }
}
...
{
    getFortuneButton->setEnabled((!networkSession || networkSession->isOpen()) && !hostCombo->currentText().isEmpty() && !portLineEdit->text().isEmpty());
}
...
void Client::sessionOpened()
{
    ...
    statusLabel->setText(tr("You must run the Fortune Server as well."));
